 ...
 */
public class LongMap<V> implements Serializable, Map<V> {
    ...
    public V[] values(Class<V> clazz) {
        V[] values = (V[])Array.newInstance(clazz,(int)size);
        int currentValues = 0;
        for(int i=0;i<table.length;i++) {
            if (table[i] != null) {
                values[currentValues] = table[i].value;
                currentValues++;
            }
        }
